...
var scrollDivs = () => {
	...
	self.dragResizeDiv = (e) => {				
		e.preventDefault();										
			if(self.activeElement.classList.contains('corner4')){
				...
				//console.log(elementPositions.x > (window.innerWidth - elementPositions.width),self.activeElement.style.left);																					
			}
	...
	self.mouseUp = (e) => {				
		...
		if(self.activeElement){
			self.activeElement.classList.remove('addBorder');
			 self.activeElement.parentNode.classList.remove('addBorder');					
		}
		console.log(self.allDivsInfo); 
	}
	...
	self.mouseDown = (e) => {					
		...
		if(self.activeElement.classList.contains('resizableCorner')){
			...
			self.activeElement.parentNode.classList.add('addBorder');
		}else{
			self.activeElement.classList.add('addBorder');
		}					
	}
	...
	self.createNewBox = (arrayVal) => {	
		...
		self.resizeDiv.style.backgroundColor = self.resizeDivPosition.bgColor;
		...
		self.close.addEventListener('mousedown',eraseElement);											
	}	
